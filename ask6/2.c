...
int main(){
    ...
    while(1){
        ...
        if(first == '5' && second == '6'){ //check if numbers are 5 and 6 accordingly(number of team))
            ...
            PORTB = 0x00;                  //Turn leds PB0-5 OFF
            _delay_ms(1000);               //wait an additional 1 second(5 seconds total)
        }
        else {
            for (int i=0; i<10; i++){      //blink leds PB0-5 for 5 seconds
                PORTB=0x3F;                //2Hz, 50% duty cycle
                _delay_ms(250);
                PORTB=0x00;
                _delay_ms(250);
            }
